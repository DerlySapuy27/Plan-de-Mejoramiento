...
if (strlen($_SESSION['alogin']) == "") {
    ...
    if (isset($_POST['submit'])) {
        ...
        $roolid = $_POST['documento'];
        $studentemail = $_POST['emailid'];
        $gender = $_POST['gender'];
        $classid = $_POST['class'];
        $dob = $_POST['dob'];
        $status = $_POST['status'];
        $sql = "update tblstudents set StudentName=:studentname,documento=:documento,StudentEmail=:studentemail,Gender=:gender,DOB=:dob,Status=:status where StudentId=:stid ";
        $query = $dbh->prepare($sql);
        $query->bindParam(':studentname', $studentname, PDO::PARAM_STR);
        $query->bindParam(':documento', $roolid, PDO::PARAM_STR);
                                    ...
                                    <form class="form-horizontal" method="post">
                                        ...
                                        if ($query->rowCount() > 0) {
                                            foreach ($results as $result) {  ?>
                                                ...
                                                <div class="form-group">
                                                    <label for="default" class="col-sm-2 control-label">Documento</label>
                                                    <div class="col-sm-10">
                                                        <input type="number" name="documento" class="form-control" id="documento" value="<?php echo htmlentities($result->RollId) ?>" maxlength="10" required="required" autocomplete="off">
                                                    </div>
                                                </div>
